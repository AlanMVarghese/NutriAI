...
function App() {
  ...
  async function generateAnswer() {
    ...
    try {
      ...
      // Fetch related food images from Unsplash
      fetchPhotos();
  ...
  async function fetchPhotos(item) {
    try {
      const result = await axios.get(API_URL, {
        params: {
          client_id: 'QkU9gopAsyjMHGJvaYKNwJJPo5yg-D8bwWF4453_-r4', // Use environment variable
          query: 'rice',  // You can adjust this based on the diet plan
        },
      });
      setImageUrl(result.data.results[0]?.urls?.thumb || '');  // Use the first image result
    } catch (error) {
      console.error('Error fetching photos:', error);
    }
  }

  // Format the answer generated by AI
  const formatAnswer = (text) => {
    ...
    return sections.slice(1).map((section, index) => {
      ...
      const items = section.slice(section.indexOf('\n') + 1).split('*').map(item => item.trim()).filter(Boolean); // Food items
  ...
  return (
    <div className='page'>
      ...
      {imageUrl && <img src={imageUrl} alt="related to diet plan" />}
    </div>
  );
}
